...
surface<void,2> sur_A;
surface<void,2> sur_B;
...
mm_kernel( float *B,float* C, int wA, int wB)
{
  ...
  for (int a = aBegin, b = bBegin;
    ...
    surf2Dread(&AS(ty,tx),sur_A,int((a+wA*ty+tx)%wA*4),int((a+wA*ty+tx)/wA),cudaBoundaryModeTrap);// tex2D(tex_A,(a+wA*ty+tx)%wA,(a+wA*ty+tx)/wA);//A[a + wA * ty + tx];
    surf2Dread(&BS(ty,tx),sur_B,(b+wB*ty+tx)%wB*4,(b+wB*ty+tx)/wB,cudaBoundaryModeTrap);//tex2D(tex_B,(b+wB*ty+tx)%wB,(b+wB*ty+tx)/wB);//B[b + wB * ty + tx];
