...
texture<char,1,cudaReadModeElementType> tex_C;
...
mm_kernel( float* C, int wA, int wB)
{
  ...
  for (int a = aBegin, b = bBegin;
    ...
    AS(ty, tx) = tex1Dfetch(tex_A,a+wA*ty+tx);
    BS(ty, tx) =tex1Dfetch(tex_B, b + wB * ty + tx);//tex2D(tex_B,(b+wB*ty+tx)%wB,(b+wB*ty+tx)/wB);//B[b + wB * ty + tx];
