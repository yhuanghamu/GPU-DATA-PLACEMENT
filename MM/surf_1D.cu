...
#define iSizeMultiple 2 //must be multipes of 15
...
surface<void,1> sur_A;
surface<void,1> sur_B;
surface<void,2> sur_C;
...
mm_kernel( float* C, int wA, int wB)
{
  ...
  for (int a = aBegin, b = bBegin;
    ...
    surf1Dread(&AS(ty,tx),sur_A,int((a+wA*ty+tx)*4),cudaBoundaryModeTrap);// tex2D(tex_A,(a+wA*ty+tx)%wA,(a+wA*ty+tx)/wA);//A[a + wA * ty + tx];
    surf1Dread(&BS(ty,tx),sur_B,(b+wB*ty+tx)*4,cudaBoundaryModeTrap);//tex2D(tex_B,(b+wB*ty+tx)%wB,(b+wB*ty+tx)/wB);//B[b + wB * ty + tx];
...
//  C[c + wB * ty + tx] = Csub;
surf2Dwrite(Csub,sur_C,(c + wB * ty + tx)%wB*4,(c + wB * ty + tx)/wB,cudaBoundaryModeTrap);
...
printf("size A = %d bytes,size B=%d bytes,size C=%d bytes\n",mem_size_A,mem_size_B,mem_size_C);
  ...
  checkCudaErrors(cudaMalloc((void**) &d_B, mem_size_B));
   ...
   cudaChannelFormatDesc channelDescA =  cudaCreateChannelDesc(32, 0, 0, 0, cudaChannelFormatKindFloat);//cudaCreateChannelDesc<float>();
   cudaChannelFormatDesc channelDescB =  cudaCreateChannelDesc(32, 0, 0, 0, cudaChannelFormatKindFloat);//cudaCreateChannelDesc<float>();
cudaChannelFormatDesc channelDescC =  cudaCreateChannelDesc(32, 0, 0, 0, cudaChannelFormatKindFloat);//cudaCreateChannelDesc<float>();
    cudaArray* A_Array, *B_Array,*C_Array;
    cudaMallocArray(&A_Array, &channelDescA, uiWA* uiHA,1,cudaArraySurfaceLoadStore);
    cudaMallocArray(&B_Array, &channelDescB, 262148/4,1,cudaArraySurfaceLoadStore);
cudaMallocArray(&C_Array, &channelDescC, uiWC, uiHC,cudaArraySurfaceLoadStore);
    // Copy to device memory some data located at address h_data
    // in host memory 
    cudaMemcpyToArray(A_Array, 0, 0, h_A, uiWA * uiHA * sizeof(float),
                      cudaMemcpyHostToDevice);
    cudaMemcpyToArray(B_Array, 0, 0, h_B, 262148,
                      cudaMemcpyHostToDevice);
cudaBindSurfaceToArray(sur_A,A_Array,channelDescA);
cudaBindSurfaceToArray(sur_B,B_Array,channelDescB);
cudaBindSurfaceToArray(sur_C,C_Array,channelDescC);
...
cudaMemcpyFromArray(h_C,C_Array,0,0,uiWC*uiHC*sizeof(float),cudaMemcpyDeviceToHost);
