...
uint smid=get_smid();
   if (xIndex < width && yIndex < height)
   {
       ...
       if (smid==0){
          printf("0 0 0 0 %d %p %d\n",threadIdx.y*blockDim.x+threadIdx.x,&idata[index_in],blockIdx.y*gridDim.x+blockIdx.x);
         // printf("%d 1 1 0 0 %d %d\n", yIndex*blockDim.x+xIndex,index_out);
       }
   }
}
...
int main(int argc, char **argv) {
 ...
 printf("dimension 2 2\n");
 printf("sizeof %d %d\n",sizeof(float),sizeof(float));
 printf("block_X %d %d\n",trans_size_x,trans_size_y);
 printf("block_Y %d %d\n", trans_size_y,trans_size_x);
 printf("array_size %d %d\n",trans_mem_size,trans_mem_size);
