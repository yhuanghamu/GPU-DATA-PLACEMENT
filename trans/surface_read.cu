...
//	}
//surf2Dwrite(tex1Dfetch(tex_1DA,index_in),surf,yIndex*sizeof(float),xIndex,cudaBoundaryModeTrap);
	}
}
...
int main(int argc, char **argv) {
 ...
 cudaChannelFormatDesc forB=cudaCreateChannelDesc(32,0,0,0,cudaChannelFormatKindFloat);
cudaChannelFormatDesc forC=cudaCreateChannelDesc(32,0,0,0,cudaChannelFormatKindFloat);
  cudaArray* A_Array;
  cudaArray* B_Array;
cudaArray* C_Array;
//cudaMallocArray(&C_Array,&forC,trans_size_x*trans_size_y,1,cudaArraySurfaceLoadStore);
...
//cudaMemcpyToArray(C_Array,0,0,h_trans_idata,trans_mem_size,cudaMemcpyHostToDevice);
//cudaBindSurfaceToArray(surf_1DA,C_Array,forC);
cudaMemcpyToArray(B_Array,0,0,h_trans_idata,trans_mem_size,cudaMemcpyHostToDevice);
