...
__global__ void trans_kernel(float *odata, cudaArray* idata, int width, int height)
{
   ...
   if (xIndex < width && yIndex < height)
   {
       unsigned int index_in  = xIndex + width * yIndex;
       unsigned int index_out = yIndex + height * xIndex;
       odata[index_out] = idata[xIndex][yIndex];//tex2D(tex_A,index_in%width,index_in/width);//idata[index_in];
   }
}
...
//cudaBindTexture(0,tex_A,d_trans_idata,trans_mem_size);
  ...
  trans_kernel<<<trans_grid, trans_block>>>(d_trans_odata, A_Array, trans_size_x, trans_size_y);
