...
texture<float,1,cudaReadModeElementType> tex1D_A;
...
__global__ void trans_kernel(float *odata, float* idata, int width, int height,int p0)
{
...
float _temp1;
if(p0==0)
	_temp1=idata[index_in];
else if(p0==1)
	_temp1=tex1Dfetch(tex1D_A,index_in);
else if(p0==2)
	_temp1=tex2D(tex_A,index_in%width,index_in/width);
else if(p0==3)
	_temp1=__ldg(&idata[index_in]);
	
       odata[index_out] = _temp1;//tex2D(tex_A,index_in%width,index_in/width);//idata[index_in];
   }
}
...
int main(int argc, char **argv) {
 ...
 cudaBindTextureToArray(tex_A, A_Array, channelDescA);
  ...
  trans_kernel<<<trans_grid, trans_block>>>(d_trans_odata, d_trans_idata, trans_size_x, trans_size_y,2);
