                                 ...
                                 const int inum)
{
  ...
  float4 ipos = tex1Dfetch(tex_position,idx);
  ...
  while (j < neighCount)
  {
    int jidx = tex1Dfetch(tex_neighbor,j*inum+idx);//neighList[j*inum + idx];
    float4 jpos;
    jpos = tex1Dfetch(tex_position,jidx);
...
//  cudaSetDevice(1);
  ...
  cudaBindTexture(0,tex_neighbor,d_md_neighborList,md_nAtom*md_maxNeighbors*sizeof(int));  
