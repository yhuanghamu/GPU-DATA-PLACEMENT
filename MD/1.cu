...
texture<int,1,cudaReadModeElementType> tex_Nei;
                                 ...
                                 const int inum)
{
  ...
  while (j < neighCount)
  {
    int jidx = tex1Dfetch(tex_Nei,j*inum+idx);//neighList[j*inum + idx];
    float3 jpos;
    jpos = position[jidx];
...
//  cudaSetDevice(1);
  ...
  cudaBindTexture(0,tex_Nei,d_md_neighborList,sizeof(int)*md_maxNeighbors*md_nAtom);
  ...
  int md_gridSize  = (md_nAtom-1+md_BLOCK_SIZE) / md_BLOCK_SIZE;
    md_kernel<<<md_gridSize, md_BLOCK_SIZE>>>
                  (d_md_force, d_md_position, md_maxNeighbors,
