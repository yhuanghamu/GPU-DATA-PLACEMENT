...
#define md_NBLOCKS 16*6*2*2*2*2*2
...
if(sm==0)
printf("1 0 0 %d %d %p\n",j,threadIdx.x,&neighList[j*inum+idx]);
    float3 jpos;
    jpos = position[jidx];
if(sm==0)
printf("0 0 1 %d %d %p\n",j,threadIdx.x,&position[jidx]);
...
printf("dimension 1 1 1");
printf("sizeof %d %d %d\n", sizeof(float3),sizeof(int),sizeof(float3));
printf("array_size %d %d %d\n",md_nAtom*sizeof(float3),md_nAtom*md_maxNeighbors*sizeof(int),md_nAtom*sizeof(float3));
