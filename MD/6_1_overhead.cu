...
texture<int,1,cudaReadModeElementType> tex_neighList;
texture<float4,1,cudaReadModeElementType> tex_position;
inline int * md_myBuildNeighborList_blkSchedule(const int nAtom, float4* position,
...
bool md_checkResults(float4* d_force, float4* position, int *neighList, int nAtom)
{
  for (int i = 0; i < nAtom; i++)
  {
...
__global__ void md_kernel(float4*  force3,
                                ...
                                int *gotoRead_only)
{
 ...
 if(gotoGlobal[0])
  _temp1=position[idx];
 else if(gotoTexture[0])
  _temp1=tex1Dfetch(tex_position,idx);
 else if(gotoRead_only[0])
  _temp1=__ldg(&position[idx]);
  float4 ipos = _temp1;//tex1Dfetch(tex_position,idx);
...
int _temp2;
    if(gotoGlobal[1])
  _temp2=neighList[j*inum+idx];
 else if(gotoTexture[1])
  _temp2=tex1Dfetch(tex_neighList,j*inum+idx);
 else if(gotoRead_only[1])
  _temp2=__ldg(&neighList[j*inum+idx]);
    int jidx = _temp2;//tex1Dfetch(tex_neighList,j*inum+idx);//neighList[j*inum + idx];
    float4 jpos;
float4 _temp3;
 if(gotoGlobal[0])
  _temp3=position[jidx];
 else if(gotoTexture[0])
  _temp3=tex1Dfetch(tex_position,jidx);
 else if(gotoRead_only[0])
  _temp3=__ldg(&position[jidx]);
    jpos = _temp3;//tex1Dfetch(tex_position,jidx);
...
//  cudaSetDevice(1);
  ...
  cudaMallocHost((void**)&md_position, md_nAtom*sizeof(float4));
  cudaMallocHost((void**)&md_force,    md_nAtom*sizeof(float4));
  cudaMallocHost((void**)&md_neighborList, md_nAtom*md_maxNeighbors*sizeof(int));

  // Allocate device memory for position and force
  float4* d_md_force;
  float4* d_md_position;
  cudaMalloc((void**)&d_md_force, md_nAtom*sizeof(float4));
  cudaMemset(d_md_force, 0, md_nAtom*sizeof(float4));
  cudaMalloc((void**)&d_md_position, md_nAtom*sizeof(float4));
  ...
  cudaMemcpy(d_md_position, md_position, md_nAtom*sizeof(float4), cudaMemcpyHostToDevice);
  cudaBindTexture(0,tex_position,d_md_position,md_nAtom*sizeof(float4));
  cudaBindTexture(0,tex_neighList,d_md_neighborList,md_nAtom*md_maxNeighbors*sizeof(int));
int *Global=(int *)malloc(3*sizeof(int)) ;
int *Texture = (int *)malloc(3*sizeof(int)) ;
int *Read_only = (int *)malloc(3*sizeof(int)) ;
Global[0]=0;Global[1]=0;Global[2]=1;
Texture[0]=1;Texture[1]=1;Texture[2]=0;
Read_only[0]=0;Read_only[1]=0;Read_only[2]=0;
int *gotoGlobal,*gotoTexture,*gotoRead_only;
cudaMalloc((void **)&gotoGlobal,3*sizeof(int));
cudaMalloc((void **)&gotoTexture,3*sizeof(int));
cudaMalloc((void **)&gotoRead_only,3*sizeof(int));
cudaMemcpy(gotoGlobal,Global,3*sizeof(int),cudaMemcpyHostToDevice);
cudaMemcpy(gotoTexture,Texture,3*sizeof(int),cudaMemcpyHostToDevice);
cudaMemcpy(gotoRead_only,Read_only,3*sizeof(int),cudaMemcpyHostToDevice);
  ...
  int md_gridSize  = (md_nAtom-1+md_BLOCK_SIZE) / md_BLOCK_SIZE;
    md_kernel<<<md_gridSize, md_BLOCK_SIZE>>>
                  (d_md_force, d_md_position, md_maxNeighbors,d_md_neighborList,
                   md_cutsq, md_lj1, md_lj2, md_nAtom,gotoGlobal,gotoTexture,gotoRead_only);
  ...
  cudaMemcpy(md_force, d_md_force, md_nAtom*sizeof(float4),
