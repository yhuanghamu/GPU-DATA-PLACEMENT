...
__global__ void md_kernel(float3* __restrict__ force3,
                                 ...
                                 const float lj1,
                                 const float lj2,
                                 const int inum, int *index_neighborList, int *index_position)
{
  ...
  if(blockIdx.x==1&&threadIdx.x<16)
    index_position[threadIdx.x*inum+idx]+=1;
  // Force accumulator
  float3 f = {0.0f, 0.0f, 0.0f};
  ...
  while (j < neighCount)
  {
    ...
    if(blockIdx.x==1&&threadIdx.x<16)
    atomicAdd(&index_neighborList[threadIdx.x*inum*neighCount+j*inum+idx],1);
    float3 jpos;
    jpos = position[jidx];
...
int *neighborList_index=(int *)malloc(sizeof(int)*md_nAtom*md_maxNeighbors*16);
int *index_neighborList;
cudaMalloc((void **)&index_neighborList, sizeof(int)*md_nAtom*md_maxNeighbors*16);
cudaMemset(index_neighborList, 0, sizeof(int)*md_nAtom*md_maxNeighbors*16);
int *position_index=(int *)malloc(sizeof(int)*md_nAtom*16);
int *index_position;
cudaMalloc((void **)&index_position, sizeof(int)*md_nAtom*16);
cudaMemset(index_position, 0, sizeof(int)*md_nAtom*16);
  ...
  int md_gridSize  = (md_nAtom-1+md_BLOCK_SIZE) / md_BLOCK_SIZE;
    md_kernel<<<md_gridSize, md_BLOCK_SIZE>>>
                  (d_md_force, d_md_position, md_maxNeighbors, d_md_neighborList,
                   md_cutsq, md_lj1, md_lj2, md_nAtom, index_neighborList,index_position);
  ...
  cudaMemcpy(neighborList_index,index_neighborList,sizeof(int)*md_nAtom*md_maxNeighbors*16,cudaMemcpyDeviceToHost);
  cudaMemcpy(position_index,index_position,sizeof(int)*md_nAtom*16,cudaMemcpyDeviceToHost);
  FILE *f1=fopen("neighborList_D1.txt","w");
  for(int ii=0;ii<16;ii++)
   {
     for(int jj=0;jj<md_nAtom*md_maxNeighbors;jj++)
     {
       if (neighborList_index[ii*md_nAtom*md_maxNeighbors+jj]!=0)
       fprintf(f1,"%d,%d ;",jj,neighborList_index[ii*md_nAtom*md_maxNeighbors+jj]);    
       }
       
     fprintf(f1,"\n");
    }
    FILE *f2=fopen("position_D1.txt","w");
  for(int ii=0;ii<16;ii++)
   {
     for(int jj=0;jj<md_nAtom;jj++)
     {
       if (position_index[ii*md_nAtom+jj]!=0)
       fprintf(f2,"%d,%d ;",jj,position_index[ii*md_nAtom+jj]);    
       }
...
printf("Profiling results saved to \"neighborList_D1.txt\" and \"position_D1.txt\"  \n Please use analysis.py to analysis them \n");
