...
)
{
	...
	__shared__ float s_Kernel[KERNEL_LENGTH * sizeof(float)];
	for(int i = 0; i<KERNEL_LENGTH * sizeof(float);i++){
		s_Kernel[i] =c_Kernel[i];
	}
...
#pragma unroll
        ...
        for (int j = -KERNEL_RADIUS; j <= KERNEL_RADIUS; j++)
        {
            sum += s_Kernel[KERNEL_RADIUS - j] * s_Data[threadIdx.x][threadIdx.y + i * COLUMNS_BLOCKDIM_Y + j];
        }
