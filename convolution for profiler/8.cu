...
#pragma unroll
        ...
        {
            sum += tex1Dfetch(texRef,KERNEL_RADIUS - j) * s_Data[threadIdx.y][threadIdx.x + i * ROWS_BLOCKDIM_X + j];
        }
...
#pragma unroll
        ...
        for (int j = -KERNEL_RADIUS; j <= KERNEL_RADIUS; j++)
        {
            sum += tex1Dfetch(texRef,KERNEL_RADIUS - j) * s_Data[threadIdx.x][threadIdx.y + i * COLUMNS_BLOCKDIM_Y + j];
        }
