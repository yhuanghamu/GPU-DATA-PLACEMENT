...
#pragma unroll
    for (int i = ROWS_HALO_STEPS; i < ROWS_HALO_STEPS + ROWS_RESULT_STEPS; i++)
    {
       ...
       if(bx == 0&&by == 0&&flag==0)
    {
printf("1 0 0 %d %d %d\n",i-ROWS_HALO_STEPS,ty*blockDim.x+tx,baseY*pitch+baseX+i*ROWS_BLOCKDIM_X);
}
...
if(bx == 0&&by == 0&&(baseX>=-i*ROWS_BLOCKDIM_X)&&flag==0)
    
printf("1 0 1 %d %d %d\n",i,ty*blockDim.x+tx,baseY*pitch+baseX+i*ROWS_BLOCKDIM_X);
...
if(flag==0 &&bx == 0&&by == 0)//&& (ty*blockDim.x+tx)<32)
    
printf("1 0 2 %d %d %d\n",(i-ROWS_HALO_STEPS-ROWS_RESULT_STEPS),ty*blockDim.x+tx,baseY*pitch+baseX+i*ROWS_BLOCKDIM_X);
...
printf("0 0 0 %d %d %d\n",(i-ROWS_HALO_STEPS)*(2*KERNEL_RADIUS+1)+j+KERNEL_RADIUS,ty*blockDim.x+tx,KERNEL_RADIUS - j);

        }

        d_Dst[i * ROWS_BLOCKDIM_X] = sum;
if(bx == 0&&by == 0&&flag==0)//&& (ty*blockDim.x+tx)<32)
...
//cudaPrintfInit();
   ...
   getLastCudaError("convolutionRowsKernel()\n");
...
int main(int argc, char **argv)
{
    ...
    *d_Buffer;
    ...
    printf("array_size %d %d %d\n",KERNEL_LENGTH * sizeof(float),imageW * imageH * sizeof(float),imageW * imageH * sizeof(float));
