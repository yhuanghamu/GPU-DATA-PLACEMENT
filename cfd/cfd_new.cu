...
__global__ void cfd_kernel(int nelr,int* elements_surrounding_elements, const float*  
    normals,  float*  density,  float*  mx, float* my, float*  mz, float* density_energy, float* fluxes,int *d_flag)
{
	const float smoothing_coefficient = float(0.2f);
	const int i = (blockDim.x*blockIdx.x + threadIdx.x);
	...
	//float density_i = variables[i + VAR_DENSITY*nelr];  
	float density_i = density[i];//tex1Dfetch(tex_density,i);
	float3 momentum_i;
	//momentum_i.x = variables[i + (VAR_MOMENTUM+0)*nelr];
	//momentum_i.y = variables[i + (VAR_MOMENTUM+1)*nelr];
	//momentum_i.z = variables[i + (VAR_MOMENTUM+2)*nelr];
  momentum_i.x = mx[i];
  momentum_i.y = my[i];//tex1Dfetch(tex_my,i);//my[i];
  momentum_i.z = mz[i];//tex1Dfetch(tex_mz,i); //mz[i];
    ...
    // |Z for neighbor 0, Z for neighbor 1, ... |
		...
		if(nb >= 0) 	// a legitimate neighbor
		{
			...
			//momentum_nb.y = variables[nb + (VAR_MOMENTUM+1)*nelr];
			//momentum_nb.z = variables[nb + (VAR_MOMENTUM+2)*nelr];
			density_nb = density[nb];//tex1Dfetch(tex_density,nb);
			momentum_nb.x = mx[nb];//tex1Dfetch(tex_mx,nb);//mx[nb];
			momentum_nb.y = my[nb];//tex1Dfetch(tex_my,nb);//my[nb];
			momentum_nb.z = mz[nb];//tex1Dfetch(tex_mz,nb);//mz[nb];
			//density_energy_nb = variables[nb + VAR_DENSITY_ENERGY*nelr];
