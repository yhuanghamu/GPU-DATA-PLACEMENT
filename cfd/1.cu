...
texture<int,1,cudaReadModeElementType> tex_ele;
    ...
    normals, float* density, float* mx, float* my, float* mz, float* density_energy, float* fluxes,int *d_flag)
{
  ...
  momentum_i.z = mz[i];
	...
	for(j = 0; j < cfd_maxNeighbors; j++)
	{
		nb = tex1Dfetch(tex_ele,i+j*nelr);//elements_surrounding_elements[i + j*nelr];
    //optimal layout already 
    // |X for neighbor 0, X for neighbor 1, ... | Y for neighbor 0, Y for neighbor 1, ...
    // |Z for neighbor 0, Z for neighbor 1, ... |
		normal.x = normals[i + (j + 0*cfd_maxNeighbors)*nelr]; 
...
cudaBindTexture(0,tex_ele,d_cfd_neighborList, cfd_maxNeighbors*cfd_nAtom*sizeof(int));
