...
{
    __shared__ float s_Kernel[KERNEL_LENGTH];
    __shared__ float s_Data[ROWS_BLOCKDIM_Y][(ROWS_RESULT_STEPS + 2 * ROWS_HALO_STEPS) * ROWS_BLOCKDIM_X];
   if(threadIdx.y*blockDim.x+threadIdx.x<KERNEL_LENGTH)
    {s_Kernel[threadIdx.y*blockDim.x+threadIdx.x]=cc_Kernel[threadIdx.y*blockDim.x+threadIdx.x];
    }//{for (int i =0;i<KERNEL_LENGTH;i++)
    //s_Kernel[i] = cc_Kernel[i];}
   __syncthreads();
...
#pragma unroll
        ...
        {
            sum += s_Kernel[KERNEL_RADIUS - j] * s_Data[threadIdx.y][threadIdx.x + i * ROWS_BLOCKDIM_X + j];
        }
...
int main(int argc, char **argv)
{
    ...
    *d_Buffer;
    ...
    float *cc_Kernel;
    cudaMalloc((void **)&cc_Kernel,  KERNEL_LENGTH * sizeof(float));
   // cudaMemcpyToSymbol(c_Kernel, h_Kernel, KERNEL_LENGTH * sizeof(float)); 
   cudaMemcpy(cc_Kernel,h_Kernel,KERNEL_LENGTH * sizeof(float),cudaMemcpyHostToDevice); 
