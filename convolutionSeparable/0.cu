...
__device__ float *kernel;
__constant__ float c_Kernel[KERNEL_LENGTH];
void setConvolutionKernel(float *h_Kernel)
{
    cudaMemcpyToSymbol(c_Kernel, h_Kernel, KERNEL_LENGTH * sizeof(float));
	 cudaMemcpy(kernel, h_Kernel, KERNEL_LENGTH * sizeof(float),cudaMemcpyHostToDevice);
}
...
__device__ float g_Data[ROWS_BLOCKDIM_Y*((ROWS_RESULT_STEPS + 2 * ROWS_HALO_STEPS) * ROWS_BLOCKDIM_X)];
...
#pragma unroll
    ...
    {
        g_Data[threadIdx.y*((ROWS_RESULT_STEPS + 2 * ROWS_HALO_STEPS) * ROWS_BLOCKDIM_X)+threadIdx.x + i * ROWS_BLOCKDIM_X] = d_Src[i * ROWS_BLOCKDIM_X];
    }
...
#pragma unroll
    ...
    {
        g_Data[threadIdx.y*((ROWS_RESULT_STEPS + 2 * ROWS_HALO_STEPS) * ROWS_BLOCKDIM_X)+threadIdx.x + i * ROWS_BLOCKDIM_X] = (baseX >= -i * ROWS_BLOCKDIM_X) ? d_Src[i * ROWS_BLOCKDIM_X] : 0;
    }
...
#pragma unroll
    ...
    {
        g_Data[threadIdx.y*((ROWS_RESULT_STEPS + 2 * ROWS_HALO_STEPS) * ROWS_BLOCKDIM_X)+threadIdx.x + i * ROWS_BLOCKDIM_X] = (imageW - baseX > i * ROWS_BLOCKDIM_X) ? d_Src[i * ROWS_BLOCKDIM_X] : 0;
    }
...
#pragma unroll
        ...
        {
            sum += kernel[KERNEL_RADIUS - j] * g_Data[threadIdx.y*((ROWS_RESULT_STEPS + 2 * ROWS_HALO_STEPS) * ROWS_BLOCKDIM_X)+threadIdx.x + i * ROWS_BLOCKDIM_X + j];
        }
