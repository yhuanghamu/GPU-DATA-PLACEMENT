...
print("###########################")
start = time.time()
constant_size = int(sys.argv[1])
con_lat = int(sys.argv[2])
...
L1_hit = 0
L2_hit = 0
collect_cache_line_L1 = []
collect_cache_line_L2 = []
while x<len(stride):
 ...
 if (array == array_next):
   ...
   if (exp == exp_next):
    ...
    if (loop == loop_next):
     if ((warp/32) == (warp_next/32)):
      ...
      index = index_next
      if index_next*sizeof[array_next]/L1_cache_line in L1_old and index_next not in collect and  math.fabs(len(L1_old)-L1_old.index(index_next*sizeof[array_next]/L1_cache_line)) <= L1_lines:
       L1_hit += 1
       if index_next*sizeof[array_next]/L1_cache_line not in collect_cache_line_L1:
        L1.remove(index_next*sizeof[array_next]/L1_cache_line)
        L1.append(index_next*sizeof[array_next]/L1_cache_line)
        collect_cache_line_L1.append(index_next*sizeof[array_next]/L1_cache_line)
      else:
       if index_next*sizeof[array_next]/L1_cache_line not in collect_cache_line_L1:
        if index_next*sizeof[array_next]/L1_cache_line in L1_old:
         L1.remove(index_next*sizeof[array_next]/L1_cache_line)
        L1.append(index_next*sizeof[array_next]/L1_cache_line)
        collect_cache_line_L1.append(index_next*sizeof[array_next]/L1_cache_line)
       print(L2_old)
       if index_next*sizeof[array_next]/L2_cache_line in L2_old and index_next not in collect and math.fabs(len(L2_old)-L2_old.index(index_next*sizeof[array_next]/L2_cache_line)) <= L2_lines:
        L2_hit += 1
        
        if index_next*sizeof[array_next]/L2_cache_line not in collect_cache_line_L2:
         L2.remove(index_next*sizeof[array_next]/L2_cache_line)
         L2.append(index_next*sizeof[array_next]/L2_cache_line)
         collect_cache_line_L2.append(index_next*sizeof[array_next]/L2_cache_line)
       elif index_next not in collect:
        if index_next*sizeof[array_next]/L2_cache_line not in collect_cache_line_L2:
         if index_next*sizeof[array_next]/L2_cache_line in L2_old:
          L2.remove(index_next*sizeof[array_next]/L2_cache_line)
         L2.append(index_next*sizeof[array_next]/L2_cache_line)
         collect_cache_line_L2.append(index_next*sizeof[array_next]/L2_cache_line)
    ...
    if index_next*sizeof[array_next]/L1_cache_line in L1_old and index_next not in collect and  math.fabs(len(L1_old)-L1_old.index(index_next*sizeof[array_next]/L1_cache_line)) <= L1_lines:
     ...
     if index_next*sizeof[array_next]/L1_cache_line in L1_old:
      L1.remove(index_next*sizeof[array_next]/L1_cache_line)
     L1.append(index_next*sizeof[array_next]/L1_cache_line)
     if index_next*sizeof[array_next]/L2_cache_line in L2_old and \
        index_next*sizeof[array_next]/cache_line not in collect_cache_line and \
        math.fabs(len(L2_old)-L2_old.index(index_next*sizeof[array_next]/L2_cache_line)) <= L2_lines:
      L2_hit += 1
      L2.remove(index_next*sizeof[array_next]/L2_cache_line)
      L2.append(index_next*sizeof[array_next]/L2_cache_line)
     else:
  ...
  for i in access_cache_line:
   total_cache_line += i
  if(array_size[array]>constant_size):
  ...
  printout = printout +str(total)+" "+str(L1_hit)+" "+str(L2_hit)+" "
  array = array_next
  access = []
  L1 = []
  L2 = []
  ...
  L1.append(index_next*sizeof[array_next]/L1_cache_line)
  L2.append(index_next*sizeof[array_next]/L2_cache_line)
  #L1_old.append(index_next*sizeof[array_next]/L1_cache_line)
  #L2_old.append(index_next*sizeof[array_next]/L2_cache_line)
  exp = exp_next
  loop = loop_next
  warp = warp_next
  total = 0
...
for i in access_cache_line:
 total_cache_line += i 
if(array_size[array_next]>constant_size):
 total_cache_line = 99999999
 total = 99999999
