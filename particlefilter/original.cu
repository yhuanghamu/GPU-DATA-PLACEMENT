...
void check(double * arrayX, double * arrayY, double * CDF, double * u, double * xj, double * yj, int Nparticles, int num_blocks, int threads_per_block ){
	//int block_id = blockIdx.x;// + gridDim.x * blockIdx.y;num_blocks, threads_per_block
	//int i = blockDim.x * block_id + threadIdx.x;
	for(int block_id=0;block_id<num_blocks; block_id++)
	  for (int i=block_id*threads_per_block;i<block_id*threads_per_block+threads_per_block;i++){
	if(i < Nparticles){
		...
		if((xj[i] != arrayX[index])||(yj[i] != arrayY[index])) { printf("Kernel check failed! i=%d\n,%f",i,xj[i]);return ;}
...
int main(int argc, char * argv[]){
	...
	int IszX=128, IszY=128, Nfr=2, Nparticles=1000;
	...
	for(k = 1; k < Nfr; k++){
     ...
     //END_KERNEL_CALL
      //START_POST_KERNEL
		...
		check(arrayX,arrayY,CDF,u,xj,yj,Nparticles,num_blocks, threads_per_block);
		/*for(int yy=0;yy<Nparticles;yy++)
		printf("xj[%d]=%lf\n",xj[yy]);*/
