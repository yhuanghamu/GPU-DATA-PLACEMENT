...
* param7: Nparticles
*****************************/
__global__ void kernel(double * arrayX, double * arrayY, double * CDF, double * u, double * xj, double * yj, int Nparticles,int * CDF_index,int *u_index, int *arrayX_index, int *arrayY_index){
...
// printf("Nparticles=%d\n",Nparticles);	
	if(i < Nparticles){
		...
		for(x = 0; x < Nparticles; x++){
                 ...
                 atomicAdd(&CDF_index[threadIdx.x*Nparticles+x],1);
		 atomicAdd(&u_index[threadIdx.x*Nparticles+i],1);}	
                  if(CDF[x] >= u[i]){
				index = x;
				break;
			}
		...
		xj[i] = arrayX[index];
               if(blockIdx.x==1&&threadIdx.x<32){
                 atomicAdd(&arrayX_index[threadIdx.x*Nparticles+index],1);
                atomicAdd(&arrayY_index[threadIdx.x*Nparticles+index],1);
                     }
...
void particleFilter(int * I, int IszX, int IszY, int Nfr, int * seed, int Nparticles){
	...
	for(k = 1; k < Nfr; k++){
		...
		int num_blocks = ceil((double) Nparticles/(double) threads_per_block);
                int *CDF_index,*u_index,*arrayX_index,*arrayY_index;
                
                cudaMalloc((void **)&arrayX_index,32*sizeof(int)*Nparticles);
                cudaMemset(arrayX_index,0,32*sizeof(int)*Nparticles);
                
                cudaMalloc((void **)&arrayY_index,32*sizeof(int)*Nparticles);
                cudaMemset(arrayY_index,0,32*sizeof(int)*Nparticles);

                cudaMalloc((void **)&CDF_index,32*sizeof(int)*Nparticles);
                cudaMemset(CDF_index,0,32*sizeof(int)*Nparticles);
                cudaMalloc((void **)&u_index,32*sizeof(int)*Nparticles);
                cudaMemset(u_index,0,32*sizeof(int)*Nparticles);
		//KERNEL FUNCTION CALL
                long long end_copy = get_time();

		kernel <<< num_blocks, threads_per_block >>> (arrayX_GPU, arrayY_GPU, CDF_GPU, u_GPU, xj_GPU, yj_GPU, Nparticles,CDF_index,u_index,arrayX_index,arrayY_index);
                cudaThreadSynchronize();
               
                long long start_copy_back = get_time();
		//CUDA memory copying back from GPU to CPU memory
                int *index_CDF=(int *)malloc(32*sizeof(int)*Nparticles);
                int *index_u=(int *)malloc(32*sizeof(int)*Nparticles);
                int *index_arrayX=(int *)malloc(32*sizeof(int)*Nparticles);
                int *index_arrayY=(int *)malloc(32*sizeof(int)*Nparticles);

                cudaMemcpy(index_arrayX, arrayX_index, 32*sizeof(int)*Nparticles, cudaMemcpyDeviceToHost);
                cudaMemcpy(index_arrayY, arrayY_index, 32*sizeof(int)*Nparticles, cudaMemcpyDeviceToHost);
                cudaMemcpy(index_u, u_index, 32*sizeof(int)*Nparticles, cudaMemcpyDeviceToHost);
	        cudaMemcpy(index_CDF, CDF_index, 32*sizeof(int)*Nparticles, cudaMemcpyDeviceToHost);
	        FILE *f1=fopen("CDF_D1.txt","w");
	        FILE *f2=fopen("u_D1.txt","w");
	        FILE *f3=fopen("arrayX_D1.txt","w");
	        FILE *f4=fopen("arrayY_D1.txt","w");
                for(int ii=0;ii<16;ii++){
                
                 for(int jj=0;jj<1000;jj++)
                   {
                   if(index_CDF[ii*Nparticles+jj]!=0)
                    fprintf(f1,"%d,%d ;",jj,index_CDF[ii*Nparticles+jj]);
                  }
                  fprintf(f1,"\n");
                 }
                 for(int ii=0;ii<16;ii++){
                 ...
                 for(int jj=0;jj<1000;jj++)
                   {
                   if(index_u[ii*Nparticles+jj]!=0)
                    fprintf(f2,"%d,%d ;",jj,index_u[ii*Nparticles+jj]);
                  }
                  fprintf(f2,"\n");
                 }
                for(int ii=0;ii<16;ii++){
                 ...
                 for(int jj=0;jj<1000;jj++)
                   {
                   if(index_arrayX[ii*Nparticles+jj]!=0)
                    fprintf(f3,"%d,%d ;",jj,index_arrayX[ii*Nparticles+jj]);
                  }
                  fprintf(f3,"\n");
                 }
                for(int ii=0;ii<16;ii++){
                 ...
                 for(int jj=0;jj<1000;jj++)
                   {
                   if(index_arrayY[ii*Nparticles+jj]!=0)
                    fprintf(f4,"%d,%d ;",jj,index_arrayY[ii*Nparticles+jj]);
                  }
                  fprintf(f4,"\n");
                 }
...
int main(int argc, char * argv[]){
	...
	printf("Profiling results saved to \"CDF_D1.txt\", \"arrayX_D1.txt\", \"arrayY_D1.txt\" and \"u_D1.txt \"\n Please use analysis.py to analysis them \n");
	return 0;
}
