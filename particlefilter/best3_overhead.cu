...
texture<double,1,cudaReadModeElementType> tex_u;
texture<double,1,cudaReadModeElementType> tex_arrayX;
texture<double,1,cudaReadModeElementType> tex_arrayY;
/**
...
//if(gotoGlobal[1]==4)
//	_temp1=u[i];
/*else if(gotoGlobal[1]==5)
	_temp1=tex1Dfetch(tex_u,i);*/
//else if(gotoGlobal[1]==7)
//	_temp1=__ldg(&u[i]);
	s_u[threadIdx.x]=u[i];
	}
	 __syncthreads();
	...
	if(i < Nparticles){
		...
		int index = -1;
		int x;
double _temp2,_temp3;	
		for(x = 0; x < Nparticles; x++){

/*if(gotoGlobal[0]==0)
	_temp2=c_CDF[x];
else if(gotoGlobal[0]==1)
	_temp2=tex1Dfetch(tex_CDF,x);*/
/*else if(gotoGlobal[0]==3)
	_temp2=__ldg(&c_CDF[x]);
if(gotoGlobal[1]==0)
	_temp3=u[i];

else if(gotoGlobal[1]==1)
	_temp3=tex1Dfetch(tex_u,x);
else if(gotoGlobal[1]==3)
	_temp3=__ldg(&u[i]);
else if(gotoGlobal[1]>=4)
	_temp3=s_u[threadIdx.x];
_temp2=c_CDF[x];
_temp3=u[i];//s_u[threadIdx.x];*/
...
if(gotoGlobal[2]==0)
	_temp2=arrayX[index];
/*else if(gotoGlobal[2]==1)
	_temp2=tex1Dfetch(tex_arrayX,index);*/
/*else if(gotoGlobal[2]==3)
	_temp2=__ldg(&arrayX[index]);	*/
//_temp2=arrayX[index];	
		xj[i] = _temp2;
if(gotoGlobal[3]==0)
	_temp3=arrayY[index];
/*else if(gotoGlobal[3]==1)
	_temp2=tex1Dfetch(tex_arrayY,index);*/
/*else if(gotoGlobal[3]==3)
	_temp3=__ldg(&arrayY[index]);	*/
//_temp3=arrayY[index];
		yj[i] = _temp3;
...
Global[0]=0;Global[1]=4;Global[2]=0;Global[3]=0;
...
cudaMemcpyToSymbol(gotoGlobal,Global,4*sizeof(int));
		...
		kernel <<< num_blocks, threads_per_block,128*8 >>> (arrayX_GPU, arrayY_GPU, Address, u_GPU, xj_GPU, yj_GPU, Nparticles);
