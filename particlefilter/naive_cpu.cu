...
void kernel_cpu(double * arrayX, double * arrayY, double * CDF, double * u, double * xj, double * yj, int Nparticles){
        int block_id = 0;// + gridDim.x * blockIdx.y;
FILE *f = fopen("hha.txt","w");
   for(int tx=0;tx<128;tx++)
{
         int i = 128 * block_id + tx;
        ...
        if(i < Nparticles){
                ...
                if(index == -1){
                  ...
                  {fprintf(f,"2 0 0 0 %d %d\n",i,index);
                        ...
                        fprintf(f,"4 1 0 0 %d %d\n",i,i);
                        fprintf(f,"5 1 0 0 %d %d\n",i,i);
                        }
...
* param7: Nparticles
*****************************/
__global__ void kernel(int *P,int *count,double * arrayX, double * arrayY, double * CDF, double * u, double * xj, double * yj, int Nparticles){
	...
	if(i < Nparticles){
		...
		for(x = 0; x < Nparticles; x++){
/*if(blockIdx.x==0){
P[*count*6]=0;P[*count*6+1]=0;P[*count*6+2]=0;P[*count*6+3]=x;P[*count*6+4]=threadIdx.x;P[*count*6+5]=x;
atomicAdd(count,1);
P[*count*6]=0;P[*count*6+1]=0;P[*count*6+2]=0;P[*count*6+3]=x;P[*count*6+4]=threadIdx.x;P[*count*6+5]=x;
atomicAdd(count,1);
}*/
...
int *count,*P,*h_count;
int *P_h=(int *)malloc(sizeof(int)*6*10000);
h_count=(int *)malloc(sizeof(int));
cudaMalloc((void **)&P,10000*6*sizeof(int));
cudaMalloc((void **)&count, sizeof(int));
*h_count = 0;
cudaMemcpy(h_count, count, sizeof(int), cudaMemcpyHostToDevice);
		...
		clock_gettime(CLOCK_MONOTONIC,&t3);
                kernel_cpu(arrayX,arrayY,CDF,u,xj,yj,Nparticles);
...
printf("profiling time: %f\n", (t4.tv_sec-t3.tv_sec+(t4.tv_nsec-t3.tv_nsec)/1.e9));
		...
		kernel <<< 1, threads_per_block >>> (P,count,arrayX_GPU, arrayY_GPU, CDF_GPU, u_GPU, xj_GPU, yj_GPU, Nparticles);
cudaMemcpy(h_count, count, sizeof(int), cudaMemcpyDeviceToHost);
cudaMemcpy(P_h, P, sizeof(int)*(*h_count), cudaMemcpyDeviceToHost);
