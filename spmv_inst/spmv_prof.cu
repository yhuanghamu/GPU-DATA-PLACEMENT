                       ...
                       const int dim, float * out)
{
  ...
  if (myRow < dim) 
  {
   ...
   if(smid==14){
   printf("0 0 0 0 %d %p\n",blockIdx.x*blockDim.x+threadIdx.x,&rowDelimiters[myRow]);
   printf("0 0 1 0 %d %p\n",blockIdx.x*blockDim.x+threadIdx.x,&rowDelimiters[myRow+1]);
    }
    ...
    for (int j = warpStart + id; j < warpEnd; j += WARP_SIZE)
    {
      ...
      mySum += val[j] * vec[col];
if(smid==14){
   printf("1 0 0 %d %d %p\n",(j-warpStart)/WARP_SIZE,blockIdx.x*blockDim.x+threadIdx.x,&cols[j]);
   printf("2 0 0 %d %d %p\n",(j-warpStart)/WARP_SIZE,blockIdx.x*blockDim.x+threadIdx.x,&vec[col]);
   printf("3 0 0 %d %d %p\n",(j-warpStart)/WARP_SIZE,blockIdx.x*blockDim.x+threadIdx.x,&val[j]);
	}
    }
    partialSums[t] = mySum;
...
cudaSetDevice(0);
  ...
  float maxval = 200.0;
  printf("dimension 1 1 1 1\n");
  printf("sizeof %d %d %d %d\n",sizeof(int),sizeof(int),sizeof(float),sizeof(float));
  printf("array_size %d %d %d %d\n",(spmv_numRows + 1) * sizeof(int),spmv_nItems * sizeof(int),spmv_numRows * sizeof(float),spmv_nItems * sizeof(float));
